...
export default async function SettingsPage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-ink">Configuracoes da farmacia</h1>
      <form action={saveSettings} className="panel mt-6 grid gap-5 p-5 md:grid-cols-2">
        ...
        <Field label="Razao social" name="legalName" defaultValue={settings.legalName || ""} />
        <Field label="CNPJ" name="cnpj" defaultValue={settings.cnpj || ""} />
        <Field label="Endereco completo" name="address" defaultValue={settings.address || ""} />
        ...
        <Field label="Horario de funcionamento" name="openingHours" defaultValue={settings.openingHours || ""} />
        <Field label="Farmaceutico responsavel" name="pharmacistName" defaultValue={settings.pharmacistName || ""} />
        <Field label="CRF" name="pharmacistCrf" defaultValue={settings.pharmacistCrf || ""} />
        <Field label="Licenca/Alvara sanitario" name="sanitaryLicense" defaultValue={settings.sanitaryLicense || ""} />
        ...
        <Area label="Mensagem padrao de WhatsApp" name="whatsappDefaultText" defaultValue={settings.whatsappDefaultText} />
        <Area label="Avisos sanitarios exibidos no site" name="sanitaryNotice" defaultValue={settings.sanitaryNotice} />
